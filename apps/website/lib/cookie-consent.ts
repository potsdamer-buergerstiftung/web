...
export function parseConsentFromCookieValue(cookieValue: string | undefined | null): CookieConsent | null {
...
export function getCookieValueFromDocumentCookie(cookieName: string): string | null {
...
export function readConsentFromBrowser(): CookieConsent | null {
  const cookieValue = getCookieValueFromDocumentCookie(COOKIE_CONSENT_COOKIE_NAME)
...
export function writeConsentToBrowser(consent: CookieConsent): void {
  ...
  try {
    localStorage.setItem(COOKIE_CONSENT_LOCALSTORAGE_KEY, JSON.stringify(consent))
  } catch {
    // ignore
  }
