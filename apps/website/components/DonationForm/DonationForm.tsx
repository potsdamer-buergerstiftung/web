...
function DonationFormInner({ config }: { config: DonationFormConfig }) {
  ...
  useEffect(() => {
    trackUmami("donation_step_viewed", { step: donationProgress });
              ...
              <div className="flex gap-6">
                {mobileTabs.map((tab) => (
                  <button
                    key={tab.key}
                    type="button"
                    data-umami-event={`donation_step_${tab.key.toLowerCase()}`}
                    onClick={() => {
                      trackUmami("donation_step_clicked", { step: tab.key });
...
export default function DonationForm(props: DonationFormProps) {
  ...
  const initialValues = useMemo(() => {
    const initialDuration =
      config.amount.defaultDuration ??
      config.amount.durations[0]?.key ??
      "ONE_TIME";
