...
export default function DonationForm(props: DonationFormProps) {
  ...
  return (
    <div className="bg-emerald-50 rounded-lg p-8 lg:p-16">
      ...
      <h1 className="text-center font-header text-4xl font-bold">Spenden</h1>
      <p className="text-center max-w-xl mx-auto mt-4">
        Wir legen bei unseren Projekten großen Wert darauf, dass sie nachhaltig
        wirken. Das ist jedoch nur möglich, wenn wir langfristig planen und
        fördern können. Mit Deiner regelmäßigen und verlässlichen Unterstützung
      ...
      <div className="flex flex-col md:flex-row mt-16 gap-10">
        <ul className="hidden md:flex flex-col items-end shrink-0">
          {!disableProjectSelection && (
            <ProgressButton
              ...
              onClick={onProjectSelectionClicked}
            >
          ...
          <ProgressButton
            ...
            onClick={onAmountSelectionClicked}
          >
            Betrag wählen
          </ProgressButton>
        ...
        <div className="w-full">
          {donationProgress === "PROJECT_SELECTION" && (
            <DonationFormProjectSelection />
          )}
          {donationProgress === "AMOUNT_SELECTION" && (
            <DonationFormAmountSelection />
          )}
