...
export default defineLayout<LayoutOptions, LayoutQuery>({
  ...
  setup(props, { emit }) {
    ...
    function useItemOptions() {
      ...
      const fieldsDefaultValue = computed(() => {
        return fieldsInCollection.value
          .filter(
            (field) =>
              !field.meta?.hidden && !field.meta?.special?.includes("no-data"),
          )
          .slice(0, 4)
          .map(({ field }) => field)
          .sort();
      ...
      const fieldsWithRelational = computed(() => {
        ...
        return adjustFieldsForDisplays(fields.value, props.collection, system);
      });
      ...
      const activeFields = computed<(Field & { key: string })[]>({
        get() {
          ...
          return fields.value
            ...
            .filter(
              (f) => f && f.meta?.special?.includes("no-data") !== true,
            ) as (Field & { key: string })[];
        },
        set(val) {
        ...
        get() {
          return activeFields.value.map((field) => {
            ...
            return {
              ...
              field: {
                display:
                  field.meta?.display || getDefaultDisplayForType(field.type),
              ...
              sortable:
                ["json", "alias", "presentation", "translations"].includes(
                  field.type,
                ) === false,
            } as HeaderRaw;
          });
    ...
    function useTreeViewFieldsToQuery({
      ...
      const fieldsToQuery = computed(() => {
        ...
        function addSortField() {
          if (sortField.value && !fieldsToQuery.includes(sortField.value)) {
    ...
    function useFilteringTreeView({
      ...
      const isFiltered = computed(() => !!filterUser.value || !!search.value);
      ...
      async function saveEdits(edits: Record<PrimaryKey, Item>) {
        try {
          for (const [id, payload] of Object.entries(edits)) {
            await api.patch(`${getEndpoint(collection.value!)}/${id}`, payload);
          }
        } catch (error: any) {
