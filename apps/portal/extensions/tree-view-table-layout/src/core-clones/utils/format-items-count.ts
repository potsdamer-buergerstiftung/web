import type { ComposerNumberFormatting, ComposerTranslation } from "vue-i18n";
import { useI18n } from "vue-i18n";

export interface FormatItemsCountPaginatedOptions {
  currentItems: number;
  currentPage: number;
  perPage: number;
  isFiltered?: boolean;
...
export function formatItemsCountPaginated({
  ...
  if (isFiltered && (totalItems === undefined || currentItems < totalItems)) {
    if (currentItems <= perPage)
      return t("filtered_item_count", values, currentItems);
...
export interface FormatItemsCountRelativeOptions {
  totalItems: number;
  currentItems: number;
  isFiltered?: boolean;
  i18n?: { t: ComposerTranslation; n: ComposerNumberFormatting };
...
export function formatItemsCountRelative({
  ...
  // If filter is active and has effect on items
  if (isFiltered && currentItems < totalItems)
    return t("filtered_item_count", values, currentItems);
