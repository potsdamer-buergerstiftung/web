    ...
    for (const field of _fields) {
      ...
      if (fieldNameCount[fieldName]! > 1 === false) {
        aliasedFields[field] = {
          key: field,
          fieldName,
          fields: adjustFieldsForDisplays([field], _collection, system),
          aliased: false,
        };
      } else {
        ...
        aliasedFields[alias] = {
          ...
          fieldName,
          fieldAlias: alias,
          fields: adjustFieldsForDisplays([field], _collection, system).map(
            (displayField) => {
              return displayField.includes(".")
                ? `${alias}.${displayField.split(".").slice(1).join(".")}`
                : alias;
            },
          ),
          aliased: true,
